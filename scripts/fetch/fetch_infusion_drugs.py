...
INFUSION_DRUGS = [
    ...
    {"id": 372, "name": "灭菌注射用水", "url": ""},  # 无湖南药事服务网链接
]
...
def main():
    ...
    for i, drug in enumerate(INFUSION_DRUGS, 1):
        print(f"\n[{i}/{len(INFUSION_DRUGS)}] 处理: {drug['name']} (ID: {drug['id']})")
        ...
        # 添加延迟
        if i < len(INFUSION_DRUGS) and INFUSION_DRUGS[i]['url']:
            time.sleep(2)
